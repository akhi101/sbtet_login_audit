define(['app'], function (app) {
    app.controller("PreStudentRegPhotoSignListController", function ($scope, $state, AppSettings, PreStudentRegService, RegisterAdmittedStudentService) {
        $scope.PreStudentRegPhotoSignList = {};
        ...
        var gridColumns = [
            ...
            { field: "PreStudRegID", headerText: "PreStudRegID", visible: true, textAlign: ej.TextAlign.Left, width: 0 }
        ];
        $scope.StudentRegPhotoSigndata = [];
        $("#StudentRegPhotoSigns").ejGrid({
            dataSource: $scope.StudentRegPhotoSigndata,
            allowPaging: true,
        ...
        $scope.doubleclick = function doubleclick(sender, args) {
            if (this.multiSelectCtrlRequest == false) {
                $state.go('Admission.PreStudentRegPhotoSign', { PreStudRegID: sender.data.PreStudRegID });
            }
        }
        ...
        $scope.Show = function () {
            ...
            var StudentRegPhotoSigndata = PreStudentRegService.FillPreStudentRegDetailsList(AppSettings.CollegeID, $scope.CourseID, $scope.ExamID, $scope.BranchID, AppSettings.AcdYrID);
            StudentRegPhotoSigndata.then(function (data) {
                if (data.length > 0) {
                    var SrNo = 1
                    ...
                    $scope.StudentRegPhotoSigndata = data;
                }
                else {
        ...
        //All data shows
        var StudentRegPhotoSigndata = PreStudentRegService.FillPreStudentRegDetailsList(AppSettings.CollegeID, 0, 0, 0, AppSettings.AcdYrID);
        StudentRegPhotoSigndata.then(function (data) {
            if (data.length > 0) {
                var SrNo = 1
                for (var i = 0; i < data.length; i++) {
