define(['app'], function (app) {
    ...
    app.controller("GenVocPracticalMarkEntrySummeryReportController", function ($scope, $http, $localStorage, $state, AppSettings, MenuService, basicExamService, otherMarkEnrtyService, DrillDownService) {
        $scope.Admission = {};
        ...
        $scope.FillExamSubject = function (ExamID) {
            if ((ExamID != null) && (ExamID != undefined) && (ExamID != "")) {
                otherMarkEnrtyService.GetGenVocPractExamSubListForSummeryReport($scope.Admission.ExamID).then(function (results) {
                    $scope.examTimeTableSubjects = results;
                    //$scope.otherMarkEnrtyDetail.ExmSubID = "" + GetOnTheJobTrngExamSubListForMarkEntry $scope.examTimeTableSubjects[0].ExmSubID + "";GetGenVocPractExamSubListForSummeryReport
                });
            }
        }
        ...
        $scope.Submit = function () {
            ...
            if ($scope.adminuser == true || $scope.jsuser == true || $scope.coeuser == true) {
                var TotalRecorddata = DrillDownService.GetDrillDownStudentDetailsListForGeneralPractical($scope.Admission.ExamInstID, $scope.otherMarkEnrtyDetail.ExmSubID, $scope.Admission.ExamID);
                ...
                //GetDrillDownStudentDetailsListForOJTByCollege
                var TotalRecorddata = DrillDownService.GetDrillDownStudentDetailsListForGeneralPracticalByCollege($scope.Admission.ExamInstID, $scope.otherMarkEnrtyDetail.ExmSubID, AppSettings.DistrictIDs, $scope.Admission.ExamID);
                TotalRecorddata.then(function (data) {
