define(['app'], function (app) {
    app.controller("PracticalCenterAllocationGeographyController", function ($scope, $state, $stateParams, AppSettings, PreZoneCenterService, BasicDistrictsService, BasicExamService, BasicCourseService) {
        ...
        BasicCourseList.then(function (CourseData, status, headers, config, error) {
            ...
            $scope.PracticalCenterAllocation.CourseID = "1";
        ...
        $scope.GetExamList = function (CourseID) {
            if (CourseID != "" || CourseID != undefined) {
                ...
                BasicExamList.then(function (ExamData, status, headers, config, error) {
                    ...
                    $scope.PracticalCenterAllocation.ExamID = "2";
                }, function (ExamData, status, headers, config) {
                    alert(error);
                })
            }
        }
        $scope.PracticalCenterAllocationProcess = function () {
            $scope.LoadImg = true;
            $scope.isupdatableDisable = true;
            if (CheckValidation() == true) {
                ...
                var getPromise = PreZoneCenterService.PostPracticalCenterAllocationProcessGeography($scope.PracticalCenterAllocation);
        ...
        function RedirectToListPage() {
            $scope.isupdatableDisable = false;
            $state.go('CenterManagemnet.PracticalCenterAllocationGeography');
        }
    });
});
