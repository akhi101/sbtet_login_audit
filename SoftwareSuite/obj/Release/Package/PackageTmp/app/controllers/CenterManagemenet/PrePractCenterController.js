define(['app'], function (app) {
    app.controller("PrePractCenterController", function ($scope, $state, $stateParams, AppSettings, PrePractCenterService, BasicDistrictsService, BasicCollegeService, BasicCourseService, BasicExamService, PreZoneService) {
        ...
        function FillData() {
            ...
            if (AppSettings.SysUsrGrpID != 23) {
                ...
                BasicDistrictList = BasicDistrictsService.GetDistrictListForCOE(AppSettings.ExamInstID,2);
            }

            BasicDistrictList.then(function (DistrictData, status, headers, config, error) {
                $scope.BasicDistrictList = DistrictData;
                if ($scope.PrePractCenter.PrePractCntrID == 0) { $scope.PrePractCenter.DistrictID = "" + $scope.BasicDistrictList[0].DistrictID + ""; }
                if (AppSettings.SysUsrGrpID == 23) { $scope.FillListDataNew($scope.PrePractCenter.DistrictID);}
                if (AppSettings.SysUsrGrpID != 23) {
                    var ZoneList = PreZoneService.GetPreZoneListByDistrictId(AppSettings.DistrictIDs, AppSettings.ExamInstID, 2);
                } else {
                    var ZoneList = PreZoneService.GetPreZoneListByDistrictIdNew($scope.PrePractCenter.DistrictID, AppSettings.ExamInstID, 2);
                }
                            ...
                            BasicExamList.then(function (ExamData, status, headers, config, error) {
                                ...
                                if ($scope.PrePractCenter.PrePractCntrID > 0) {
                                    ...
                                    PrePractCenterdata.then(function (data) {
                                        ...
                                        var ZoneList = PreZoneService.GetPreZoneListByDistrictId(data[0].DistrictID, AppSettings.ExamInstID, 2);
                                        ZoneList.then(function (ZoneData, status, headers, config, error) {
                                            $scope.PreZoneList = ZoneData;
                                            $scope.PrePractCenter.ZoneID = "" + data[0].ZoneID + "";
                                        }, function (error) {
        ...
        $scope.GetPreZoneList = function (DistrictID) {
            if (DistrictID != "" || DistrictID != undefined) {
                $scope.EnableCenterCollege = false;
                var ZoneList = PreZoneService.GetPreZoneListByDistrictIdNew(DistrictID, AppSettings.ExamInstID, 2);
                ZoneList.then(function (ZoneData, status, headers, config, error) {
                    $scope.PreZoneList = ZoneData;
                    if (AppSettings.SysUsrGrpID == 23) { $scope.FillListDataNew($scope.PrePractCenter.DistrictID); }
                }, function (ZoneData, status, headers, config) {
        ...
        $scope.SavePrePractCenter = function () {
            ...
            if (CheckValidation() == true) {
                ...
                $scope.PrePractCenter.ExamInstID = AppSettings.ExamInstID;
                $scope.PrePractCenter.ZoneType = 2;
        ...
        function RedirectToListPage() {
            ...
            FillListData();
            $scope.ResetDate();
            $state.go('CenterManagemnet.PrePractCenter');
        }
        var gridColumns = [
        ...
        function FillListData() {
            var PrePractCenterdata = PrePractCenterService.GetPrePractCenterList(AppSettings.DistrictIDs);
            PrePractCenterdata.then(function (data) {
        ...
        $scope.FillListDataNew = function (DistrictID) {
            var PrePractCenterdata = PrePractCenterService.GetPrePractCenterList(DistrictID);
            PrePractCenterdata.then(function (data) {
                $scope.PrePractCenterList = data;
            }, function (error) {
                alert(error);
