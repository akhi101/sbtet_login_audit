define(['app'], function (app) {
    app.controller("PrePractVocBridgeBatchChartReportController", function ($scope, $state, $stateParams, AppSettings, PreZoneCenterService, BasicDistrictsService, BasicExamService, BasicCourseService, PrePractCenterService) {
        ...
        $scope.PrePracticalBatchAllocationReport.ZoneType = '4';
        ...
        function GetExamList(CourseID) {
            if (CourseID != "" || CourseID != undefined) {
                ...
                BasicExamList.then(function (ExamData, status, headers, config, error) {
                    ...
                    $scope.PrePracticalBatchAllocationReport.ExamID = "4";
                }, function (ExamData, status, headers, config) {
                    alert(error);
                })
            }
        ...
        $scope.GetPracticalCenterList = function (DistrictID) {
            if (DistrictID != "" || DistrictID != undefined) {
                var PracticalCenterList = PrePractCenterService.GetPracticalCenterListByDistrictIDANDZoneType($scope.PrePracticalBatchAllocationReport.ExamInstID, 4, DistrictID);
        ...
        $scope.GetBridgePrePracticalBatchAllocationReport = function () {
            ...
            if (CheckValidation() == true) {
                var Urlstring = "api/PrePractCenter/GetBridgePracticalChartReport/?PrePractCntrID=" + $scope.PrePracticalBatchAllocationReport.PrePractCntrID + "&ExamID=" + $scope.PrePracticalBatchAllocationReport.ExamID + "&ExamInstID=" + $scope.PrePracticalBatchAllocationReport.ExamInstID + "&UpdLoginID=" + $scope.PrePracticalBatchAllocationReport.UpdLoginID + "&DistrictID=" + $scope.PrePracticalBatchAllocationReport.DistrictID + "&CourseID=" + $scope.PrePracticalBatchAllocationReport.CourseID + "&ZoneID=" + $scope.PrePracticalBatchAllocationReport.ZoneID;
                $.ajax({
        ...
        $("#reportviewer").ejReportViewer(
            {
                ...
                reportPath: "RptBridgePractBatchChartbyCenter.rdlc",
