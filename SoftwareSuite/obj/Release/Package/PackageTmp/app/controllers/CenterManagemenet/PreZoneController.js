define(['app'], function (app) {
    app.controller("PreZoneController", function ($scope, $state, $stateParams, AppSettings, PreZoneService, BasicDistrictsService, BasicMandalService, BasicCollegeService) {
        $scope.PreZone = {};
        ...
        function FillData() {
            ...
            BasicDistrictList.then(function (DistrictData, status, headers, config, error) {
                ...
                if ($scope.PreZone.ZoneID > 0) {
                    var PreZonedata = PreZoneService.GetPreZoneById($scope.PreZone.ZoneID, 1);
            ...
            if (CheckValidation() == true) {
                ...
                if ($scope.PreZone.ZoneID == 0) {
                    ...
                    getPromise.then(function (msg) {
                        ...
                        else if (error == '[{"Id":"","Message":"Already approved or forwared by higher authority, you cant edit"}]') {
        ...
        //    if (isConfirmed) {
        //        var getData = PreZoneService.DeletePreZone($scope.PreZone.ZoneID, AppSettings.LoggedUserId, 1);
        ...
        function RedirectToListPage() {
            ...
            $scope.IsDelete =false;
            FillListData();
            $state.go('CenterManagemnet.PreZone');
        }
        var gridColumns = [
            ...
            //{ field: "ZoneCode", headerText: "Clg Type", textAlign: ej.TextAlign.Left, width: 25 },
        ...
        $scope.ShowCollegesFunction = function () {
            ...
            if (($scope.PreZone.ZoneCode == undefined) || ($scope.PreZone.ZoneCode == "")) {
                alert("Enter Zone Code");
                return;
            }
            if (($scope.PreZone.ColCode == undefined) || ($scope.PreZone.ColCode == "")) {
                alert("Enter College to be Allotted to this Zone");
                return;
            }            
            ...
            if ($scope.PreZone.ColCode != "") {
                var PreZoneCollegedata = PreZoneService.GetPreZoneCollegeCount(AppSettings.ExamInstID, $scope.PreZone.ColCode, 1);
                PreZoneCollegedata.then(function (data) {
                    if (data.length > 0) {
                        //alert("It is allocated to Zone");
                        alert("College not belongs to this district.");
                        return;
                    }
                    var BasicCollegeData = BasicCollegeService.GetCollegeListByColCode($scope.PreZone.DistrictID, AppSettings.ExamInstID, $scope.PreZone.ColCode,1);
                    BasicCollegeData.then(function (data) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            //obj.ZoneCode = $scope.PreZone.ZoneCode;
        ...
        function FillListData() {
            var PreZonedata = PreZoneService.GetPreZoneList(AppSettings.ExamInstID, 1, AppSettings.DistrictIDs);
            PreZonedata.then(function (data) {
        ...
        $scope.FillListDataNew = function (DistrictID) {
            var PreZonedata = PreZoneService.GetPreZoneList(AppSettings.ExamInstID, 1, DistrictID);
            PreZonedata.then(function (data) {
                $scope.PreZoneList = data;
            }, function (error) {
                alert(error);
        ...
        $scope.DeletePreZone = function () {
            var isConfirmed = confirm("Are you sure to delete this record ?");
            if (isConfirmed) {
                //int ZoneID, int UpdLoginID, int ZoneType
                $scope.PreZone.ZoneType = "1";
                $scope.PreZone.UpdLoginID = AppSettings.LoggedUserId;
                //var getData = PreZoneService.DeletePreZone($scope.PreZone.ZoneID, AppSettings.LoggedUserId, "1");
