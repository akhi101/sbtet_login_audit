define(['app'], function (app) {
    app.controller("PrePracticalBatchAllocationBridgeController", function ($scope, $state, $stateParams, AppSettings, PreZoneCenterService, BasicDistrictsService, BasicExamService, BasicCourseService) {
        ...
        $scope.GetExamList = function (CourseID) {
            if (CourseID != "" || CourseID != undefined) {
                ...
                BasicExamList.then(function (ExamData, status, headers, config, error) {
                    ...
                    $scope.PrePracticalBatchAllocation.ExamID = "4";
                }, function (ExamData, status, headers, config) {
                    alert(error);
                })
            }
        ...
        $scope.PracticalBatchAllocatuionProcessBridge = function () {
            $scope.isupdatableDisable = true;
            $scope.LoadImg = true;
            if (CheckValidation() == true) {
                ...
                var getPromise = PreZoneCenterService.PostProcessCenterPractBatchBridge($scope.PrePracticalBatchAllocation);
        ...
        function RedirectToListPage() {
            $scope.isupdatableDisable = false;
            $state.go('CenterManagemnet.PrePracticalBatchAllocationBridge');
        }
    });
});
