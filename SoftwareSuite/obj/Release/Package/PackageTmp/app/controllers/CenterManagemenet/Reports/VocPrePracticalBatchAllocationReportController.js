define(['app'], function (app) {
    app.controller("VocPrePracticalBatchAllocationReportController", function ($scope, $state, $stateParams, AppSettings, PreZoneCenterService, BasicDistrictsService, BasicExamService, BasicCourseService, PrePractCenterService) {
        ...
        $scope.GetPreZoneList = function (DistrictID) {
            if (DistrictID != "" || DistrictID != undefined) {
                ...
                //var ZoneList = PreZoneCenterService.GetPreZoneListByDistrictId(DistrictID, AppSettings.ExamInstID, 3);
                if ($scope.PrePracticalBatchAllocationReport.CourseID == 1) {
                    $scope.PrePracticalBatchAllocationReport.ZoneType = 2;
                }
                else if ($scope.PrePracticalBatchAllocationReport.CourseID == 2) {
                ...
                var ZoneList = PreZoneCenterService.GetPreZoneListByDistrictId(DistrictID, AppSettings.ExamInstID, $scope.PrePracticalBatchAllocationReport.ZoneType);
                ZoneList.then(function (ZoneData, status, headers, config, error) {
                    $scope.PreZoneList = ZoneData;
                }, function (ZoneData, status, headers, config) {
                    alert(error);
        ...
        BasicCourseList.then(function (CourseData, status, headers, config, error) {
            ...
            $scope.PrePracticalBatchAllocationReport.CourseID = "2";
        ...
        function GetExamList(CourseID) {
            if (CourseID != "" || CourseID != undefined) {
                ...
                BasicExamList.then(function (ExamData, status, headers, config, error) {
                    ...
                    if ($scope.PrePracticalBatchAllocationReport.DistrictID == "" || $scope.PrePracticalBatchAllocationReport.DistrictID == undefined) {
                        $scope.PrePracticalBatchAllocationReport.DistrictID = 0;
                    }
                }, function (ExamData, status, headers, config) {
                    alert(error);
                })
        ...
        $scope.GetPracticalCenterList = function (ZoneID) {
            if ($scope.PrePracticalBatchAllocationReport.DistrictID != "" || $scope.PrePracticalBatchAllocationReport.DistrictID != undefined) {
                var PracticalCenterList = PrePractCenterService.GetPrePractCenterByDistrictIdByZoneID($scope.PrePracticalBatchAllocationReport.DistrictID, $scope.PrePracticalBatchAllocationReport.ExamInstID, $scope.PrePracticalBatchAllocationReport.ZoneID, $scope.PrePracticalBatchAllocationReport.ZoneType);
        ...
        $scope.GetPrePracticalBatchAllocationReport = function () {

            if ($scope.PrePracticalBatchAllocationReport.ZoneID == undefined || $scope.PrePracticalBatchAllocationReport.ZoneID == "") {
                $scope.PrePracticalBatchAllocationReport.ZoneID = 0;
            }
            ...
            if (CheckValidation() == true) {
                    var Urlstring = "api/PrePractCenter/GetCenterPractBatchReportChart/?PrePractCntrID=" + $scope.PrePracticalBatchAllocationReport.PrePractCntrID + "&ExamID=" + $scope.PrePracticalBatchAllocationReport.ExamID + "&ExamInstID=" + $scope.PrePracticalBatchAllocationReport.ExamInstID + "&UpdLoginID=" + $scope.PrePracticalBatchAllocationReport.UpdLoginID + "&DistrictID=" + $scope.PrePracticalBatchAllocationReport.DistrictID + "&CourseID=" + $scope.PrePracticalBatchAllocationReport.CourseID + "&ZoneID=" + $scope.PrePracticalBatchAllocationReport.ZoneID;
                    $.ajax({
        ...
        $("#reportviewer").ejReportViewer(
            {
                ...
                reportPath: "RptVocationPracticalBatchCenterWise.rdlc",
