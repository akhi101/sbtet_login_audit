define(['app'], function (app) {
    app.controller("PreZoneVocationalController", function ($scope, $state, $stateParams, AppSettings, PreZoneService, BasicDistrictsService, BasicMandalService, BasicCollegeService) {
        $scope.PreZone = {};
        ...
        function FillData() {
            ...
            BasicDistrictList.then(function (DistrictData, status, headers, config, error) {
                ...
                if ($scope.PreZone.ZoneID > 0) {
                    var PreZonedata = PreZoneService.GetPreZoneById($scope.PreZone.ZoneID, 3);
        ...
        $scope.DeletePreZone = function () {
            ...
            if (isConfirmed) {
                var getData = PreZoneService.DeletePreZone($scope.PreZone.ZoneID, AppSettings.LoggedUserId, 3);
        ...
        function RedirectToListPage() {
            ...
            $scope.IsDelete = false;
            FillListData();
            $state.go('CenterManagemnet.PreZoneVocational');
        }
        var gridColumns = [
            ...
            { field: "ColName", headerText: "College Name", textAlign: ej.TextAlign.Left, width: 80 },
            ...
            { field: "CourseCount2", headerText: "Course Count(II yr)", textAlign: ej.TextAlign.Left, width: 80 },
        ];
        $scope.BasicCollegeList = [];
        $("#BasicCollege").ejGrid({
            ...
            toolbarClick: function (args) {
                if (args.itemName == "Delete") {
                    var isConfirmed = confirm("Are you sure to delete this record ?");
                    if (isConfirmed) {
                        //var DelCollegeID = args.model.selectedRecords[0].CollegeID;
                        //var PreCheckCenterData = PreZoneService.GetPreCheckCenterData($scope.PreZone.ZoneID, DelCollegeID, AppSettings.ExamInstID, 3);
                        //PreZonedata.then(function (data) {
                        //    if (data == 1) {
                        //        args.cancel();
                        //    } else {
        ...
        $scope.ShowCollegesFunction = function () {
            ...
            if ($scope.PreZone.ColCode != "") {
                var PreZoneCollegedata = PreZoneService.GetPreZoneCollegeCount(AppSettings.ExamInstID, $scope.PreZone.ColCode, 3);
                PreZoneCollegedata.then(function (data) {
                    if (data.length > 0) {
                        alert("It is allocated to Zone");
                        return;
                    }
                    var PreZoneCollegedataStream = PreZoneService.GetCheckStreamOfCollege($scope.PreZone.ColCode, 2);
                    PreZoneCollegedataStream.then(function (data) {
                        if (data == 0) {
                            alert("Vocational Stream not available in this College");
                            return;
                        }
                        var BasicCollegeData = BasicCollegeService.GetCollegeListByColCode($scope.PreZone.DistrictID, AppSettings.ExamInstID, $scope.PreZone.ColCode,3);
                        BasicCollegeData.then(function (data) {
                            ...
                            for (var i = 0; i < data.length; i++) {
                                ...
                                obj.CourseCount2 = data[i].CourseCount2;
        ...
        function FillListData() {
            var PreZonedata = PreZoneService.GetPreZoneList(AppSettings.ExamInstID, 3, AppSettings.DistrictIDs);
            PreZonedata.then(function (data) {
        ...
        $scope.FillListDataNew = function (DistrictID) {
            var PreZonedata = PreZoneService.GetPreZoneList(AppSettings.ExamInstID, 3, DistrictID);
            PreZonedata.then(function (data) {
                $scope.PreZoneList = data;
            }, function (error) {
                alert(error);
