define(['app'], function (app) {
    app.controller("GroupChangeListController", function ($scope, $state, AppSettings, StudentRegService, RegisterAdmittedStudentService) {
        ...
        $scope.GroupChangeListdata = [];
        $("#GroupChangeListGrid").ejGrid({
            dataSource: $scope.GroupChangeListdata,
            allowPaging: true,
        ...
        $scope.doubleclick = function doubleclick(sender, args) {
            if (this.multiSelectCtrlRequest == false) {
                $state.go('Admission.GroupChange', { StudRegID: sender.data.StudRegID });
            }
        }
        ...
        $scope.Show = function () {
            ...
            var GroupChangedata = StudentRegService.FillStudentRegDetailsListForGroupChange(AppSettings.CollegeID, $scope.CourseID, $scope.ExamID, $scope.BranchID);
            GroupChangedata.then(function (data) {
                if (data.length > 0) {
                    var SrNo = 1
                    ...
                    $scope.GroupChangeListdata = data;
                }
                else {
        ...
        //All data shows
        var GroupChangedata = StudentRegService.FillStudentRegDetailsListForGroupChange(AppSettings.CollegeID, 0, 0, 0);
        GroupChangedata.then(function (data) {
            if (data.length > 0) {
                var SrNo = 1
                for (var i = 0; i < data.length; i++) {
