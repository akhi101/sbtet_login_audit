define(['app'], function (app) {
    app.controller("StudentRegPhotoSignListController", function ($scope, $state, AppSettings, StudentRegService, RegisterAdmittedStudentService) {
        $scope.StudentRegPhotoSignList = {};
        ...
        var gridColumns = [
            ...
            { field: "PRNNo", headerText: "PRNNo", visible: false, textAlign: ej.TextAlign.Right, width: 0 }
        ];
        ...
        $scope.doubleclick = function doubleclick(sender, args) {
            if (this.multiSelectCtrlRequest == false) {
                if (sender.data.PRNNo != "") {
                    alert("PRN is generated, you can't edit record.");
                    return;
                } else {
                    $state.go('Admission.StudentRegPhotoSign', { StudRegID: sender.data.StudRegID });
                }
            }
        }
        ...
        $scope.Show = function () {
            ...
            var StudentRegPhotoSigndata = StudentRegService.FillStudentRegDetailsListForPhotoUpload(AppSettings.CollegeID, $scope.CourseID, $scope.ExamID, $scope.BranchID, $scope.WithPhoto);
        ...
        //All data shows
        var StudentRegPhotoSigndata = StudentRegService.FillStudentRegDetailsListForPhotoUpload(AppSettings.CollegeID, 0, 0, 0, $scope.WithPhoto);
        StudentRegPhotoSigndata.then(function (data) {
            if (data.length > 0) {
                var SrNo = 1
                for (var i = 0; i < data.length; i++) {
