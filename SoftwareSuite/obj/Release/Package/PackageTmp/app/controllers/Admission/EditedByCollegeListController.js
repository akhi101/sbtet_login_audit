define(['app'], function (app) {
    app.controller("EditedByCollegeListController", function ($scope, $state, AppSettings, EditedByCollegeService, RegisterAdmittedStudentService) {
        $scope.EditedByCollegeList = {};
        ...
        $scope.EditedByCollegedata = [];
        $("#EditedByColleges").ejGrid({
            dataSource: $scope.EditedByCollegedata,
            allowPaging: true,
        ...
        $scope.doubleclick = function doubleclick(sender, args) {
            if (this.multiSelectCtrlRequest == false) {
                $state.go('Admission.EditedByCollege', { PreStudRegID: sender.data.PreStudRegID });
            }
        }
        ...
        $scope.Show = function () {
            ...
            var EditedByCollegedata = EditedByCollegeService.GetDataForEditByCollegeList(AppSettings.CollegeID, $scope.CourseID, $scope.ExamID, $scope.BranchID);
            EditedByCollegedata.then(function (data) {
                if (data.length > 0) {
                    var SrNo = 1
                    ...
                    $scope.EditedByCollegedata = data;
                }
                else {
        ...
        //All data shows
        var EditedByCollegedata = EditedByCollegeService.GetDataForEditByCollegeList(AppSettings.CollegeID, 0, 0, 0);
        EditedByCollegedata.then(function (data) {
            if (data.length > 0) {
                var SrNo = 1
                for (var i = 0; i < data.length; i++) {
