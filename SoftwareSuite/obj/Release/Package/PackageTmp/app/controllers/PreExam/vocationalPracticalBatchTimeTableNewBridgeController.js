(function () {
    'use strict';
    angular.module('app')
        .controller('vocationalPracticalBatchTimeTableNewBridgeController',
            function ($rootScope, $scope, examTimeTableService, basicCourseService, basicDistrictsService, BasicMainGroupService, PrePractCenterService, AppSettings) {
                ...
                $scope.$watch('vocationalPracticalBatchTimeTable.PrePractCntrID', function () {
                    if ($scope.vocationalPracticalBatchTimeTable.PrePractCntrID !== undefined) {
                        ...
                        PrePractCenterService.getExamTimeTableSubjectListByExamIDBranchIDForBridge().then(function (results) {
                            $scope.basicSubjectList = results;
                        });
                        $scope.vocationalPracticalBatchTimeTable.ExmSubID = 0;
                        if ($scope.vocationalPracticalBatchTimeTable.cheExaminerAll == true) {
                            PrePractCenterService.GetExamineForVocationalPracticalForBridge(0, $scope.vocationalPracticalBatchTimeTable.ExmSubID).then(function (results) {
                            ...
                            PrePractCenterService.GetExamineForVocationalPracticalForBridge($scope.vocationalPracticalBatchTimeTable.DistrictID, $scope.vocationalPracticalBatchTimeTable.ExmSubID).then(function (results) {
                                $scope.BasicExaminerListForChange = results;
                                if ($scope.BasicExaminerListForChange.length > 0) {
                                    $scope.vocationalPracticalBatchTimeTable.ExaminerCode = $scope.BasicExaminerListForChange[0].ExaminerCode;
                                } else {
                ...
                $scope.GetPracticalBatchesByGroup = function () {
                    ...
                    if ($scope.vocationalPracticalBatchTimeTable.DistrictID == undefined) { $scope.vocationalPracticalBatchTimeTable.DistrictID = 0; }
                    if ($scope.vocationalPracticalBatchTimeTable.ExmSubID == undefined) { $scope.vocationalPracticalBatchTimeTable.ExmSubID = 0; }
                    var PracticalBatchesData = PrePractCenterService.GetBatchListForVocationalPracticalForBridge($scope.vocationalPracticalBatchTimeTable.DistrictID,
                ...
                $scope.saveVocatinalBatchExamTimeTable = function () {
                    ...
                    //if (($scope.vocationalPracticalBatchTimeTable.ExmSubID == undefined) || ($scope.vocationalPracticalBatchTimeTable.ExmSubID == "")) {
                    ...
                    for (var i = 0; i < $scope.PracticalBatchesList.length; i++) {
                        ...
                        if ($scope.isValid) {
                            if ($scope.PracticalBatchesList[i].AppointMentDate != null) {
                                ...
                                $scope.tempDetails.ExmSubID = $scope.PracticalBatchesList[i].ExmSubID;
                    ...
                    if ($scope.preVocBatchesList.length > 0 && $scope.isValid) {
                        ...
                        examTimeTableService.PutvocationalPracticalBatchTimeTableBridge($scope.vocationalPracticalBatchTimeTable).then(function (results) {
