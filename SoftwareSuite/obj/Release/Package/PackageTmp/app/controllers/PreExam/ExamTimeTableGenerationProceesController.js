define(['app'], function (app) {
    app.controller("ExamTimeTableGenerationProceesController", function ($scope, $state, $stateParams, AppSettings, BasicCourseService, BasicEvalTypesService, examTimeTableService) {
        $scope.ExamTimeTable = {};
        ...
        $scope.GenerateExamTimeTable = function ()
        {
            if ($("#StartDate").val() != "") { $scope.ExamTimeTable.StartDate = $("#StartDate").val(); }
            //if ($scope.ExamTimeTable.length > 0) {
            examTimeTableService.postProcessGenerateTheoryExamTimeTable($scope.ExamTimeTable).then(function (results) {
