define(['app'], function (app) {
    app.controller("PrctExmCenterWiseStMentController", function ($scope, $filter, $state, $interval, $stateParams, AppSettings, BasicDistrictsService) {
        $scope.PreExmSpelPract = {};
        $scope.isDistrictDisable = false;

        $scope.GetCenterWisePracticalExamStatement = function () {
            if (CheckValidation() == true) {
                if ($scope.PreExmSpelPract.DistrictID == undefined || $scope.PreExmSpelPract.DistrictID == "") {
                    $scope.PreExmSpelPract.DistrictID = 0;
                }
                var Urlstring = "api/PreExmSpelPract/GetPracticalSpellCenterWiseData/?DistrictID=" + $scope.PreExmSpelPract.DistrictID + "&ExamInstID=" + AppSettings.ExamInstID + "";
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        var datasetName1 = "dsPracticalStateMentCenterWise";
                        if (data[0].length == 0) {
        ...
        $("#container").ejReportViewer(
            {
                ...
                reportPath: "RptPracticalCenterWiseStateMentReport.rdlc",
                dataSources: [{ value: [], name: "dsPracticalStateMentCenterWise" }],
