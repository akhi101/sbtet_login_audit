define(['app'], function (app) {
    app.controller("MC_StudentDetailsController", function ($scope, $http, $localStorage, $uibModal, $state, AppSettings, PreExaminationService) {
        ...
        $scope.Approve = function () {
            var ApproveStatus = 1
            var Approve = PreExaminationService.MigrationSetApproveStatus($scope.Certificate, $scope.pin, $scope.UserTypeId, ApproveStatus);
        ...
        $scope.Submit = function (remarks) {
            ...
            var Approve = PreExaminationService.MigrationSetApproveStatusReject($scope.Certificate, $scope.pin, $scope.UserTypeId, ApproveStatus, remarks);
            Approve.then(function (response) {
                var response = JSON.parse(response)
                if (response.Table[0].ResponseCode == '200') {
                    alert(response.Table[0].ResponseDescription)
