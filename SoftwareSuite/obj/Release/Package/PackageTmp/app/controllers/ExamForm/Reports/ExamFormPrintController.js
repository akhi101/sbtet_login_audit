define(['app'], function (app) {
    app.controller("ExamFormStudentCountController", function ($scope, $state, AppSettings, ExamFormsService, ExamFormsApprovalService, BasicCourseService, BasicExamService) {
        ...
        $scope.ExamFormPrint.ApprovalType = "A";
        ...
        if (AppSettings.CollegeID == 0) {
            ...
            $scope.ExamFormPrint.CollegeID = AppSettings.CollegeID;
        }
        ...
        $scope.FillCollege = function (MandalID) {
            if ((MandalID > 0) || (MandalID != undefined)) {
                ...
                $scope.ExamList = [];
                $scope.CollegeList = [];
                var CollegeList = ExamFormsApprovalService.GetCollegeListByDistrictAndMandal($scope.ExamFormPrint.DistrictID, MandalID);
        ...
        $scope.FillCollData = function (CollegeID) {
            if (CollegeID != null) {
                ...
                $scope.BranchList = [];
                $scope.ExamList = [];
                var CourseList = ExamFormsApprovalService.GetCourseListForRegStud($scope.ExamFormPrint.CollegeID);
        ...
        $scope.Print = function () {
            if (AppSettings.CollegeID == 0) {
                if (($scope.ExamFormPrint.CollegeID == undefined) || ($scope.ExamFormPrint.CollegeID == "")) {
                    alert("Select College");
                    return false;
                }
            }
            if (($scope.ExamFormPrint.CourseID == undefined) || ($scope.ExamFormPrint.CourseID == "")) {
                alert("Select stream");
                return;
            }
            if (($scope.ExamFormPrint.ExamID == undefined) || ($scope.ExamFormPrint.ExamID == "")) {
                alert("Select Exam");
                return;
            }
            ...
            //if ($scope.ExamFormPrint.ApprovalFromDate > $scope.ExamFormPrint.ApprovalToDate) {
            //    alert("Select To Date must be greater than From Date");
            //    return;
            //}
            //var Urlstring = "api/ExamForms/GetExamFormStudentCount/?AcdYrID=" + AppSettings.AcdYrID + "&CollegeID=" + $scope.ExamFormPrint.CollegeID + "&CourseID=" + $scope.ExamFormPrint.CourseID + "&ExamID=" + $scope.ExamFormPrint.ExamID + "&BranchID=" + $scope.ExamFormPrint.BranchID + "&" +
            //  "ApprovalType=" + $scope.ExamFormPrint.ApprovalType + "&ApprovalFromDate=" + $scope.ExamFormPrint.ApprovalFromDate + "&ApprovalToDate=" + $scope.ExamFormPrint.ApprovalToDate + "";
            var Urlstring = "api/ExamForms/GetExamFormStudentCount/?ExamInstID=" + AppSettings.ExamInstID + "&CollegeID=" + $scope.ExamFormPrint.CollegeID + "&CourseID=" + $scope.ExamFormPrint.CourseID + "&ExamID=" + $scope.ExamFormPrint.ExamID + "&BranchID=" + $scope.ExamFormPrint.BranchID + "&ApprovalType=" + $scope.ExamFormPrint.ApprovalType + "";
            $.ajax({
