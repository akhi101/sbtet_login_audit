define(['app'], function (app) {
    app.controller("EquiCertAppListController", function ($scope, $state, AppSettings, EquiCertAppService) {
        ...
        var gridColumns = [
            ...
            { field: "EquiCertID", headerText: "EquiCertID", visible: true, textAlign: ej.TextAlign.Right, width: 0 },
        ];
        $scope.EditData = "N";
        $scope.EquiCertAppdata = [];
        $("#EquiCertApps").ejGrid({
            dataSource: $scope.EquiCertAppdata,
            allowPaging: true,
        ...
        $scope.doubleclick = function doubleclick(sender, args) {
            if (this.multiSelectCtrlRequest == false) {
                $state.go('StudentRequestBoard.EquiCertApp', { EquiCertID: sender.data.EquiCertID });
            }
		}
		//var CourseList = EquiCertAppService.GetBasicCourseList();
		...
		//	var ExamList = EquiCertAppService.GetBasicExamList(CourseID);
		//	ExamList.then(function (BasicExamdata, status, headers, config, error) {
		//		$scope.ExamList = BasicExamdata;
		//		var BranchList = EquiCertAppService.GetBasicBranchList(CourseID);
		...
		//$scope.Show = function () {
			...
			//	EdiData = false;
			//}
			//var EligCertAppdata = EquiCertAppService.FillEquiCertAppDetailsList($scope.CourseID, $scope.ExamID, $scope.BranchID, UserGrp, EditData);
   ...
   //             }
        ...
        $scope.ChangeChange = function () {
            ...
            var EligCertAppdata = EquiCertAppService.FillEquiCertAppDetailsList(UserGrp, EditData);
            EligCertAppdata.then(function (data) {
                if (data.length > 0) {
                    var SrNo = 1
                    for (var i = 0; i < data.length; i++) {
